...
class RTCoreV3:
    ...
    """Implementation of a trainer with a basic set of features."""
    def __init__(self, context, action_controller, reward_controller, observation_builder):
        self.context = context
        self.action_controller = action_controller
    ...
    #@with_exception(RTCoreError)
    def apply_action(self, action):
        action_result = self.action_controller.apply_action(action)
        reward = self.reward_controller.apply_action(action)
