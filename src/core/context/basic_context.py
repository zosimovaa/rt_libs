...
class BasicContext:
    ...
    # todo перенести в action_controller
...
class TradeContext:
    ...
    def __init__(self, market_fee=0):
        self.data_point = None
        self.params = {self.DEFAULT_DOMAIN: dict()}
        logger.debug("Instance created with market_fee %s", market_fee)
    ...
    def reset(self):
        ...
        self.context.set("is_open_prev", False)
        logger.debug("Reset")

    def set(self, param, value, domain=DEFAULT_DOMAIN):
        if domain not in self.params:
            self.params[domain] = dict()
            logger.debug("Domain %s created", domain)
        self.params[domain][param] = value
        logger.debug("Value %s set to param %s", value, param)

    def get(self, param, default=None, domain=DEFAULT_DOMAIN):
        return self.params.get(domain, dict()).get(param, default)
