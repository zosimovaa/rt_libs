...
class RatesFeatureNorm(BaseFeature):
    ...
    def _get(self):
        data_point = self.context.get("data_point")
        data = data_point.get_values(self.price, step_factor=self.step_factor)
        norm_values = data_point.get_values(self.price, step_factor=self.step_factor, num=-1)
...
class RatesDiffFeature(BaseFeature):
    ...
    def _get(self):
        ...
        data = data_point.get_values(self.price, step_factor=self.step_factor, num=data_point.observation_len + 1)
        data = data.diff()
